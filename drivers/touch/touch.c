...
#define GT911_REG_P1_DATA   0x814FU  /* first touch point: 8 bytes (X_L,X_H,Y_L,Y_H,...) */
#define GT911_MAX_XY        32767U
...
static bool read_gt911(touch_state_t *out)
{
    ...
    uint16_t raw_x = (uint16_t)pt[0] | ((uint16_t)pt[1] << 8U);
    uint16_t raw_y = (uint16_t)pt[2] | ((uint16_t)pt[3] << 8U);
    out->x = (uint32_t)raw_x * TOUCH_DISPLAY_W / (GT911_MAX_XY + 1);
    out->y = (uint32_t)raw_y * TOUCH_DISPLAY_H / (GT911_MAX_XY + 1);
