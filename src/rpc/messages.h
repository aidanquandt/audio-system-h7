#pragma once

#include <stdint.h>
#include "ipc/ipc_shared.h"

typedef enum {
    MSG_HEARTBEAT_CM4    = 0x01,
    MSG_HEARTBEAT_CM7    = 0x02,
    MSG_LED_TOGGLE_GREEN = 0x10,  /* handled on CM4 */
    MSG_LED_TOGGLE_RED   = 0x11,  /* forwarded to CM7 */
    MSG_SET_GAIN         = 0x20,
    MSG_PEAK_METER       = 0x80,
} msg_id_t;

typedef struct __attribute__((packed)) { uint32_t seq; } heartbeat_cm4_t;
typedef struct __attribute__((packed)) { uint32_t seq; } heartbeat_cm7_t;
typedef struct __attribute__((packed)) { uint8_t reserved; } led_toggle_green_t;
typedef struct __attribute__((packed)) { uint8_t reserved; } led_toggle_red_t;
typedef struct __attribute__((packed)) { uint8_t channel; float gain_db; } set_gain_t;
typedef struct __attribute__((packed)) { uint8_t channel; float peak_db; } peak_meter_t;

_Static_assert(sizeof(heartbeat_cm4_t)  <= RPC_FRAME_MAX_PAYLOAD, "heartbeat_cm4_t exceeds RPC_FRAME_MAX_PAYLOAD");
_Static_assert(sizeof(heartbeat_cm7_t)  <= RPC_FRAME_MAX_PAYLOAD, "heartbeat_cm7_t exceeds RPC_FRAME_MAX_PAYLOAD");
_Static_assert(sizeof(set_gain_t)        <= RPC_FRAME_MAX_PAYLOAD, "set_gain_t exceeds RPC_FRAME_MAX_PAYLOAD");
_Static_assert(sizeof(peak_meter_t)      <= RPC_FRAME_MAX_PAYLOAD, "peak_meter_t exceeds RPC_FRAME_MAX_PAYLOAD");
