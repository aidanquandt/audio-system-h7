...
#ifdef CORE_CM7
    configASSERT(ipc_queue_push(&IPC_SHARED->cm7_to_cm4, &msg) == 0);
    bsp_hsem_notify(HSEM_CH_CM7_TO_CM4);
#else
    configASSERT(ipc_queue_push(&IPC_SHARED->cm4_to_cm7, &msg) == 0);
